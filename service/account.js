...
var account = {
  getAccountDetails: function(req, res, next){
    ...
    var getAccountData = function(callback) {
      req.app.db.models.Account.findById(req.user.roles.account.id, 'name company phone zip purchaseHistoryLog addressLog mailingAddress').exec(function(err, account) {
        if (err) {
    ...
    var getPHData = function(callback) {
      ...
      req.app.db.models.PurchaseHistory.find({"account.id": req.user.roles.account.id }, 'orderDate orderNumber account.name cost').exec(function(err, purchasehistory) {
        ...
        outcome.purchasehistory = purchasehistory;
        return callback(null, 'done');
      });
    };

    var getMailAddressData = function(callback) {
      req.app.db.models.Address.findOne({ "account.id" : req.user.roles.account.id, "type" : "mailing" }, 'account.name type addressLine1 addressLine2 city state zip country').exec(function(err, mailingAddress) {
        if (err) {
          callback(err, null);
        }

        outcome.mailingAddress = mailingAddress;
        return callback(null, 'done');
      });
    };

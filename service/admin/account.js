...
var account = {
  find: function (req, res, next) {
    ...
    var getResults = function (callback) {
      ...
      req.app.db.models.Account.pagedFind({
        filters: filters,
        keys: 'name company phone zip userCreated status purchaseHistoryLog user',
  ...
  purchaseHistory: function(req, res, next){
    var workflow = req.app.utility.workflow(req, res);

    workflow.on('findPH', function(err) {
      req.app.db.models.PurchaseHistory.findById(req.params.id, function(err, purchases) {
        if (err) {
          return workflow.emit('exception', err);
        }

        workflow.outcome.purchases = purchases;
        workflow.emit('response');
      });
    });
